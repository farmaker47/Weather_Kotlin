...
class FortnightlyToolbar(
    ...
    private val header1 by lazyFast { findViewById<View>(R.id.header1) }
    private val header2 by lazyFast { findViewById<View>(R.id.header2) }
    private val header3 by lazyFast { findViewById<View>(R.id.header3) }
    ...
    fun translate(amount: Int) {
        allMovement += amount
        radius = clamp(allMovement * 0.5f, 0f, maxAllowedRadius)
    }
